...
export default async(req, res) => {
    ...
    const response = await fetch('https://api.bcb.gov.br/dados/serie/bcdata.sgs.11/dados?formato=json&dataInicial=' + dataInicial + '&dataFinal=' + dataFinal);
    ...
    data.map(async dados => {
        ...
        dataSelic =  ano + '-' + ("0"+mes).slice(-2) + '-' + ("0"+dia).slice(-2);

        //valida quais dados são repetidos e separa os que são para inserir no banco
        validaData = await knex('selic')
            .where('dt_efetiva', dataSelic)
            .first()
            ...
            .then(valores => {
                if(valores === undefined){
                    arraySelic.push({dt_efetiva: dataSelic, vr_taxa: parseFloat(dados.valor)})
                }
            })
    })
    ...
    switch(method){
        ...
        case 'POST':
            ...
            try {
                await knex('selic').insert(arraySelic);
