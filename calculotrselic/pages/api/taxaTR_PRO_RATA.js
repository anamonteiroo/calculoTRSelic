...
export default async(req, res) => {
    ...
    const response = await fetch('https://api.bcb.gov.br/dados/serie/bcdata.sgs.227/dados?formato=json&dataInicial=' + dataInicial + '&dataFinal=' + dataFinal);
    ...
    data.map(async dados => {
        ...
        validaData = await knex('tr_prorata')
            .where('dt_fim', dataTR)
            .first()
    ...
    switch(method){
        ...
        case 'POST':
            ...
            try {
                await knex('tr_prorata').insert(arrayTR);
